...
if (isset($_POST['submit']) && ($_POST['action']=="add" || $_POST['action']=="update")) {
  if ($_POST['date'] != "" && $_POST['price'] != "" && $_POST['quantity'] != "") {
    if ($_POST['customer'] != 0 && $_POST['employee'] != 0 && $_POST['part']!=0) {
      if($_POST['action']=="add"){
        $partquan = $db->query('SELECT quantity FROM parts WHERE partid='.$_POST['part'].';');
        if ($_POST['quantity']>$partquan){
          $return['error'] = true;
        ...
        $partquan = $db->query('SELECT parts.quantity+partsales.quantity FROM parts, partsales WHERE parts.partid='.$_POST['part'].' AND partsales.saleid='.$_POST['sale'].';');
        if ($_POST['quantity']>$partquan){
          $return['error'] = true;
          $return['msg'] = "Trying to sell more parts than are in stock";
          echo json_encode($return);
